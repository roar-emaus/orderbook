...
def get_orderbook(identifier, market):
    url = (
        "https://www.nordnet.no/mux/popups/marknaden/"
    ...
    for element in tr_elements:
        ...
        time = dt.datetime.strptime(
            td_elements[5].split('"last">')[1].split("<img")[0].strip(), "%H:%M:%S"
        ).time()
    ...
    orderbook_frame.to_csv(
        f'../data/{ticker}_{market}_{today_date.strftime("%m_%d")}.pd',
        sep=",",
        encoding="utf-8",
        index=False,
    )
...
if __name__ == "__main__":
    ...
    for m in market_names:
        ...
        markets.append(
            get_stocks(
                "https://www.nordnet.no/mux/web/marknaden/kurslista/"
